...
.fa-sack-dollar,.fa-bag-shopping{
	color: white;
	font-size: 1.7em; /* 將圖案放大為原始大小的兩倍 */
}
...
<!-- 				<button id="txts"  class="btn btn-bubble"><a href="/admin/orderTxt.controller" id="txts" >TXT</a></button> -->
					...
					<div class="box span12">
						...
						<div class="box-content">
							...
							<table
								...
								width="100%">
								<button id="jsons"  class="btn btn-bubble"><a href="/admin/orderJson.controller" id="jsons" >JSON</a></button>
				<button id="csvs"  class="btn btn-bubble"><a href="/admin/orderCsv.controller" id="csvs" >CSV</a></button>
								<thead>
									<tr style="font-weight: bold; font-size: 23px;line-height: 2em;">
										<!-- 											style="table-layout:fixed" -->
										<th style="width: 8%">編號</th>
										<th style="width: 19%">訂單日期</th>
										<th style="width: 19%">會員姓名</th>
										<th style="width: 13%">總金額</th>
										<th style="width: 15%">詳細內容</th>
										<th style="width: 15%">狀態</th>
										<th style="width: 11%">取消訂單</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach varStatus="stVar" var="giftorder"
										items="${giftorder}">
										<tr style=" font-size: 20px;padding:0px 5px;">

											<td align="center">${giftorder.giftorderid}</td>
											<td class="center" align="center">${giftorder.orderdate}</td>
											<td class="center" align="center">${giftorder.username}</td>
											<td class="center" align="center">${giftorder.totalamount}</td>
											<td><a
												href="<c:url value='/admin/orderdetail.controller/${giftorder.giftorderid}' />"
												name="id">詳細內容</a></td>
											<td class="status"><c:choose>
													<c:when test="${giftorder.cancletag == 1}">
														<span class="cancle">&nbsp;已&nbsp;取&nbsp;消&nbsp;</span>
													</c:when>
													<c:otherwise>
														<c:if test="${giftorder.cancletag != 1}">
															<span class="established">&nbsp;成&nbsp;立&nbsp;</span>
														</c:if>
													</c:otherwise>
												</c:choose></td>
											<td>
...
// document.getElementById("jsons").addEventListener("click", function() {
	 
//     $.ajax({
//         url: "/admin/orderJson.controller",
//         method: "POST",
//         dataType: "text",
...
// });
 ...
 $(document).on('click', '#csvs', function(){
		Swal.fire({
			  position: 'center',
         title: 'CSV輸出完成!',
         icon: 'success',
...
//  $(document).on('click', '#txts', function(){
// 		Swal.fire({
// 			  position: 'center',
//       title: 'TXT輸出完成!',
//       icon: 'success',
...
// document.getElementById("csvs").addEventListener("click", function() {
//     $.ajax({
//         url: "/admin/orderCsv.controller",
//         method: "POST",
//         dataType: "text",
//         success: function(response) {
...
// document.getElementById("excels").addEventListener("click", function() {
//     $.ajax({
//         url: "/admin/orderExcel.controller",
//         method: "POST",
//         dataType: "text",
//         success: function(response) {
...
// document.getElementById("txts").addEventListener("click", function() {
//     $.ajax({
//         url: "/admin/orderTxt.controller",
//         method: "POST",
//         dataType: "text",
//         success: function(response) {
