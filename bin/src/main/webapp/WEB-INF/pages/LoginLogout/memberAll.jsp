			...
			<div class="row-fluid sortable">	
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="fa-regular fa-user"></i><span class="break"></span>會員 Members</h2>
						<div class="box-icon">
					...
					<div  style="font-size: 3em ">
						<a href="/admin/exportCsv" id="outputCsv"><i class="fa-solid fa-file-csv" style="margin-right: 5px; margin-bottom: 5px;"></i></a>
						<a href="/admin/exportPdf" id="outputPdf"><i class="fa-solid fa-file-pdf"></i></a>
						<a href="/admin/exportjson" id="outputJson"><i class="fa-solid fa-file-code"></i></a>
					</div>
...
<!-- 								  <th>身分證字號</th> -->
			...
			<tbody>
				<c:forEach var="hotelBean" items="${classList}" varStatus="vstatus">
				
					<tr>
						<td id="userid-${vstatus.index}" style="vertical-align: middle; text-align: center;">${hotelBean.userid}</td>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.username}</td>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.useremail}</td>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.userphone}</td>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.usergender}</td>
<%-- 						<td style="vertical-align: middle; text-align: center;">${hotelBean.usercountry}</td> --%>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.useraddress}</td>
<%-- 						<td style="vertical-align: middle; text-align: center;">${hotelBean.useridentity}</td> --%>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.userbirthday}</td>
						<td style="vertical-align: middle; text-align: center;">${hotelBean.userregistration}</td>
						<td style="vertical-align: middle; text-align: center;">
							   <c:choose>
								   <c:when test="${hotelBean.userstatus=='啟用'}">
								       <select name="userstatus1" id="userstatus-${vstatus.index}">
								        	<option value="啟用" selected>啟用</option>
								        	<option value="停用">停用</option>
								        </select>
								   </c:when>
								   <c:when test="${hotelBean.userstatus=='停用'}">
								        <select name="userstatus1" id="userstatus-${vstatus.index}">
								   ...
								   <c:otherwise>
								   	<select name="userstatus1" id="userstatus-${vstatus.index}">
								   	  <option value="${hotelBean.userstatus}" selected >${hotelBean.userstatus}</option>
								   	 </select>
								   </c:otherwise>
							   </c:choose>
						</td>
						<td class="center" style="display: flex;">
							<form method="Post" action="/admin/memberDetailFindById.controller" class="searchmember">
								<button class="edit" name="userid" value="${hotelBean.userid}"
									style='border: 0; background-color: transparent'>
									<i class="fa-solid fa-circle-info"></i>
								</button>
							</form>	
							<form method="Post" action="/admin/updateMemberStatus.controller" class="updatemember" style="margin:0 2px">
								<button id="editbtn123" class="searchmember editbtn123" name="userid"
									style='border: 0; background-color: transparent' value="${hotelBean.userid}">
									<i class="fa-solid fa-pen-to-square"></i>
								</button>
								<input type="hidden" name="index123" value="${vstatus.index}">
							</form>	
							<a class="deletemember">
								<button class="deletemember" value="${hotelBean.userid}" name="userid" style='border: 0; background-color: transparent'><i class="fa-solid fa-trash-can"></i></button>
							</a>
						</td>
					</tr>
				</c:forEach>
