...
<body onload="getCountries()">
	...
	<div class="offcanvas-menu-wrapper">
		...
		<nav class="mainmenu mobile-menu">
			<ul>
				...
				<li><a href="<c:url value='/users/giftdisplay.controller'/>">伴手禮</a></li>
	...
	<div class="col-xl-4 col-lg-5 offset-xl-2 offset-lg-1">
			...
			<form action='/register2.controller' enctype='multipart/form-data'
                method='Post' onsubmit="return checkAndShowSwal()">
                        <h3>註冊會員</h3>
				<div class="select-option">
					<label for="date-in">姓名:</label> <input type="text" name="username"
						id="username" required/><br/><span class="errorMessage">${usernameNull}</span>
				</div>
				<div class="select-option">
					...
					<br/><span class="errorMessage">${userpasswordNull}</span>
				</div>
				<div class="select-option">
					<label for="guest">再次確認您的密碼：</label> <input type="password"
						name="confirmpassword" id="confirmpassword" size=30
						placeholder="再次確認密碼"
						pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}" required /><br/><span class="errorMessage">${errorMsgMap.LoginError}</span>
						<br/><span class="errorMessage">${confirmpasswordNull}</span>
				</div>
				<div class="select-option">
					<label for="room">身分證字號：</label> <input type="text"
						name="useridentity" id="useridentity" maxlength="10" required /><br/><span class="errorMessage">${errorMessageIdentity}</span>
					<br/><span class="errorMessage">${useridentityNull}</span>
				</div>
				<div class="select-option">
					<label for="room">E-mail：</label> 
					<input type="email" name="email" id="email" size=30 required /><br/><span class="errorMessage">${errorMessageEmail}</span>
					<br/><span class="errorMessage">${useremailNull}</span>
				</div>
				<div class="select-option">
					<label for="room">電話號碼：</label> <input type="tel" name="userphone"
						id="userphone" maxlength="10" required />
					<br/><span class="errorMessage">${userphoneNull}</span>
				</div>
				<div class="select-option">
...
<!-- 					<select id="country" name="usercountry" required ></select> -->
				...
				<input type="button" value='一鍵輸入' id="inputform"
					class="btn btn-outline-primary"><br> <br> 
				<input style="margin-right: 40px" id="registeraccount" type="submit"
