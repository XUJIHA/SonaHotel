...
public class InsertRoomController {
	...
	@PostMapping("/insertroom.controller")
	public String processInsertAction(Model m, @RequestParam("roomId")@Nullable String InputRoomId,
...
//			return "roomType/roomInsert";
		...
		int row = rService.insertRoomType(bean);
...
//		return "roomType/roomInsert";
	...
	private RoomsBean checkInputText(String inputRoomId, String inputName, String inputStatus, String inputCapacity, String inputBed,
		...
		// 檢查房間編號
		String roomId = inputRoomId.trim(); 
		if (roomId.equals("")) {errors.put("idNull", "必填欄位不可空白!!");
		} else if (rService.idExist(roomId)) {errors.put("pkError", "編號已被使用!!");}
		// 房名留空給預設值
